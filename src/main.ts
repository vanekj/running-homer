import {
  availableMonitors as getAllMonitors,
...
async function prepareWindows() {
  ...
  const unusedWindows = [...allWindows].splice(allMonitors.length);

  await Promise.all(unusedWindows.map((appWindow) => appWindow.close()))
  await Promise.all(allMonitors.map(async (monitor, monitorIndex) => {
    const appWindow = allWindows[monitorIndex];
    await appWindow.setTitle('Running Homer');
